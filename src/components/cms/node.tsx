...
export const VisualBuilderNode: CmsLayoutComponent = ({ children, contentLink, ctx, layoutProps }) => {
	if (layoutProps?.layoutType == "experience") return children;

	const className = `vb:${layoutProps?.layoutType}`;
	if (layoutProps?.layoutType == "section")
		return (
			<CmsEditable as="div" className={className} cmsId={contentLink.key} ctx={ctx}>
				{children}
			</CmsEditable>
		);
