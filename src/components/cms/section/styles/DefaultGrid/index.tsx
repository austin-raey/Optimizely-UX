...
export const DefaultGrid: DefaultGridComponent = ({ children, data, layoutProps, ...htmlProps }) => {
	const { gridWidth = "default", sectionColor = "default", vSpacing = "default" } = extractSettings(layoutProps);
	...
	if (sectionColor == "default") {
		...
		return (
			<section className={cssClasses.filter((x) => x && x.length > 0).join(" ")} {...htmlProps}>
				{children}
			</section>
		);
	}
	...
	return (
		<section className={mainClasses.filter((x) => x && x.length > 0).join(" ")} {...htmlProps}>
